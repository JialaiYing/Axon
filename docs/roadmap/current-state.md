...
|---|---|
| **Dashboard** | Greeting header, "Today" agenda (overdue/due/scheduled/in-progress + inline goal-progress bars), an "Up next" queue separate from the agenda, a stats strip (streak/focus-today/intervals/productivity), a "Focus this week" 7-day area chart, and a bottom trio of Personal goals / Rank / Recent. |
| **Kanban** | Page is titled "Kanban" in both the page header and nav. Three columns (To Go Queue / In Progress / Finished). Cards support subtasks, attachments, dependencies, recurrence (daily/weekly/monthly), a free-text "subject" field (suggestions only, no color binding), an independent optional per-objective color swatch (renders as a left accent bar), and a priority level (low/medium/high/urgent) shown as a colored dot. 7-day soft-delete recycle bin. |
...
| **Flashcards** | Folders → Sets → Cards. Library view toggles between "Grid" and "Visual gallery" (a 3D Dome view). A separate "Home" left rail duplicates folder creation/navigation already available in the main Library panel. No spaced-repetition scheduling — cards track a manual "mastery" value only. |
...
| **Analytics** | Focus trend, completion, streak-heatmap-style charts; a "More insights" toggle reveals extra charts. No generated advice text lives here. |
...
| **Settings** | Single-file page: Profile, Appearance (theme), Dashboard backgrounds, Data & privacy (sync status, delete account), Focus Mode toggle, Notifications, Feature tips reset, Homepage link. Every block currently carries an explanatory sentence of body copy under its heading. |
