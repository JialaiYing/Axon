...
- `axon:progress:v1` — XP/level/rank/streaks
...
|---|---|
| **Dashboard** | Greeting header (New objective → `/kanban?new=1`; Start focus becomes Open timer when a session is active), optional active-timer chip, "Today" agenda hero (due-cards cue when Leitner dueCount > 0; Clear day CTAs; "+N more" overflow links), stats strip (streak / focus-today / open-today), Rank strip with daily goal glance `n/3` → `/goals`. Up next, Focus this week chart, Personal/Recent trio, inline goal bars, and the opaque 7-day productivity index removed. |
| **Board** | Page titled "Board" in header and nav (route remains `/kanban`). Three columns (To Go Queue / In Progress / Finished). Cards support subtasks, attachments, dependencies, recurrence (daily/weekly/monthly), a free-text "subject" field (suggestions only), and a **subject-owned left accent** via `colorForSubject()` (same mapping as Calendar + Analytics; no per-objective color picker). Priority (low/medium/high/urgent) shown as a colored dot. 7-day soft-delete recycle bin. |
...
| **Flashcards** | Folders → Sets → Cards. Leitner SR (boxes 1–5, `dueAt`). Library **Study** only when cards are due (caught-up state otherwise); set overview leads with due / box breakdown; **Practice all** is explicit. Cross-set Study shows set title per card; session end summary; in-place edit for set metadata + card text (schedule preserved). |
...
| **Analytics** | Focus trend, completion, streak-heatmap-style charts; a "More insights" toggle reveals extra charts. Focus-by-subject bars use the shared subject color map (`colorForSubject`). Focus-time trend chip compares the selected range to the prior equal window. No generated advice text lives here. |
...
| **Rank / Progress** | 10 named ranks × 3 tiers each (Novice → Polymath), 30 levels total. XP from: completing an objective (priority-weighted 25/50/85/130, up to +25% early-completion bonus, up to +50% streak bonus), a focus session (`max(8, minutes × 1.2)`), and a flat +15/day activity bonus. Cosmetic unlocks: dark-only app palettes — starters at L1 (Axon, Tokyo Night, Nord); gated Everforest L3 / Gruvbox L7 / Catppuccin Mocha L13. Unlock ≠ auto-equip; equip in Settings → Appearance. Specs: `theme-palettes.md`. |
| **Settings** | Single-file page: Profile, Appearance (light/dark + unlocked dark palette picker), Data & privacy (sync status, delete account), Focus Mode toggle, Notifications, Feature tips reset, Homepage link. Explanatory body copy under blocks cut to essentials (M1). |
