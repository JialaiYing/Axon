...
1. **Apply schema** — run `supabase/schema.sql` in the Supabase SQL editor (includes RLS).
2. **Env vars on the host** (Vercel → Settings → Environment Variables):
   ...
   - `SUPABASE_SERVICE_ROLE_KEY` = service role key (server-only; enables Delete account)
...
- [ ] Settings → Delete account (only if service role key is set)
