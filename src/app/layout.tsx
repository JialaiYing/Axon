      ...
      <head>
        <script
          dangerouslySetInnerHTML={{
            ...
            __html: `(function(){try{var p=window.location.pathname;var marketing=(p==='/'||p==='/login'||p==='/privacy'||p==='/terms'||p==='/faq');if(!marketing){document.documentElement.setAttribute('data-scope','dashboard');}if(marketing){document.documentElement.setAttribute('data-palette','axon');return;}var t=localStorage.getItem('axon:theme');if(t==='light'||t==='dark'){document.documentElement.setAttribute('data-theme',t);document.documentElement.style.colorScheme=t;}var all=['axon','tokyo-night','nord','everforest','gruvbox','catppuccin-mocha'];var starters=['axon','tokyo-night','nord'];function ok(id,list){return id&&list.indexOf(id)>=0;}var eff=localStorage.getItem('axon:paletteEffective');var pal=localStorage.getItem('axon:palette');var next=ok(eff,all)?eff:(ok(pal,starters)?pal:'axon');document.documentElement.setAttribute('data-palette',next);}catch(e){}})();`,
          }}
        />
      </head>
