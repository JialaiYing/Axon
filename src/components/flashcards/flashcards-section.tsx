...
export function FlashcardsSection() {
  ...
  const [mode, setMode] = React.useState<LibraryMode>({ type: "dome" });
  ...
  /** Dome exit → brief loading state → study view. */
  const openSetForStudy = React.useCallback(
    (set: FlashcardSet) => {
      ...
      loadTimerRef.current = window.setTimeout(
        ...
        prefersReducedMotion ? 0 : STUDY_LOAD_MS
      );
    },
    [prefersReducedMotion, touchSet]
  );

  const backToDome = React.useCallback(() => {
    if (loadTimerRef.current) window.clearTimeout(loadTimerRef.current);
    setMode({ type: "dome" });
  }, []);

  // If the studied set gets deleted (via the edit dialog), fall back to the dome.
  React.useEffect(() => {
    if ((mode.type === "study" || mode.type === "loading") && !studySet) {
      backToDome();
    }
  }, [mode.type, studySet, backToDome]);
  ...
  return (
    <>
      ...
      >
        ...
        {/* ── Library (3/4): dome ↔ loading ↔ study ─────────────────── */}
        <div
          className={cn(
              ...
              >
                <div className="flex items-center justify-between border-b border-white/8 bg-white/[0.02] px-4 py-2.5">
                  ...
                  <span className="text-[10px] tabular-nums text-white/35">
                    {sets.length} set{sets.length === 1 ? "" : "s"} · {folders.length} folder
                    {folders.length === 1 ? "" : "s"}
                  </span>
                </div>
